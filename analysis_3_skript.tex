...
\begin{document}
  ...
  \chapter{Äußere Maße}
    ...
    \begin{proof}
      Existenz folgt aus Satz II.13 und Satz II.7 ($\sigma(\skript{R}) \subseteq \skript{M}(\mu))$. Sei $\tilde{\mu}$ ein Maß auf $\sigma(\skript{R})$ mit $\tilde{\mu} = \lambda$ auf $\skript{R}$. Für $A_i \in \skript{R}$ und $\bigcup\limits_{i = 1}^n A_i = A \in \sigma(\skript{R})$ folgt aus Satz I.7. $\tilde{\mu}(A) = \lim\limits_{n \to \infty} \tilde{\mu}(\bigcup\limits_{i=1}^n A_i) = \lim\limits_{n\to\infty} \mu (\bigcup\limits_{i=1}^n A_i) = \mu(A)$. Für $E \in \sigma(\skript{R})$ mit $\mu(E) < \infty$ und $\epsilon > 0$ ex. Mengen $A_i \in \skript{R}, A = \bigcup\limits_{i=1}^\infty A_i$ mit $E \subseteq A$ und $\mu(A) \leq \mu(E) + \epsilon \implies \mu(A\setminus B) \leq \epsilon$. Aus $\mu(A) = \tilde{\mu}(A)$ und Lemma II.14 folgt \\    
      $\mu(E) \leq \mu(A) = \tilde{\mu}(A) = \tilde{\mu}(E) + \tilde{\mu}(A\setminus E) \leq \tilde{\mu}(E) + \mu(A\setminus E) \leq \tilde{\mu}(E) + \epsilon$. \\
      Lasse $\epsilon > 0$ und betrachte $\tilde{\mu}(E) \leq \mu(E)$(Lemma II.14) $\implies \mu(E) = \tilde{\mu}(E)$. \\ Sei nun $\lambda$ $\sigma$-endlich. Dann ex. o.B.d.A. paarweise disjunkte $X_n \in \skript{R}$ mit $\mu(X_n) < \infty$ und $X = \bigcup\limits_{n=1}^{\infty} X_n$. Für $E \in \sigma(\skript{R})$ bel. folgt: \\
      $\mu(E) = \sum_{n=1}^{\infty}\mu(E\cap X_n) = \sum_{n=1}^{\infty}\tilde{\mu}(E\cap X_n) = \tilde{\mu}(E) \implies \mu = \tilde{\mu}$ auf $\sigma(\skript{R})$.
    \end{proof}
    ...
    \begin{proof}
      ...
      \item $\mu(D) \leq \infty$: Aus Def. von Caratheodory-Fortsetzung folgt $\forall n \in \N D \subseteq E^n = \bigcup\limits_{i=1}^{\infty}A_i^n$ mit $A_i^n \in \skript{R}$  und $\sum\limits_{i=1}^{\infty}\lambda(A_i^n) \leq \mu(D) + \frac{1}{n}$. Wähle $E := \bigcap\limits_{n=1}^{\infty} E^n \implies E \in \sigma(\skript{R})$ mit $D \subseteq E$ und $\forall n \in \mathbb{N}$ gilt: \\
    ...
    \begin{proof}
      Satz II.7 $\implies \mu|_{\skript{M}(\mu)}$ ist vollständiges Maß. \\
      Satz I.10 $\implies \sigma(\skript{R})_{\mu|_{\sigma(\skript{R})}} \subseteq \skript{M}(\mu)$. Sei $D \in \skript{M}(\mu)$ mit $\mu(D) < \infty$. Wähle $E \in \sigma(\skript{R})$ mit $D \subseteq E$. \\ Aus Satz II.16 $\implies \mu(D) = \mu(E) = \mu(E\cap D) + \mu(E\setminus D) = \mu(D) + \mu(E\setminus(D)) \implies \mu(E\setminus D) = 0$. \\
      $\lambda$ $\sigma$-endlich $\implies \exists X_n \in \skript{R}$ mit $X = \bigcup\limits_{n=1}^{\infty}X_n$ und $\mu(X_n) < \infty$ $\forall n\in\mathbb{N}$. \\
      Für $D \in \skript{M}(\mu)$ bel. setze $D_n := \bigcup\limits_{k=1}^{n} D \cap X_k \implies D_n \subseteq D_{n+1}$ $\forall n \in \mathbb{N}$ mit $\mu(D_n) < \infty$, $D = \bigcup\limits_{n=1}^{\infty}D_n$. \\
      Wie bewiesen ex. $E_n \supset D_n$ mit $E_n \in \sigma(\skript{R})$ und $\mu(E_n\setminus D_n) = 0$. Für $E = \bigcup\limits_{n=1}^{\infty}E_n \supset D$ folgt $E \in \sigma(\skript{R})$ mit $\mu(E\setminus D) \leq \sum\limits_{n=1}^{\infty}\mu(E_n \setminus D_n)=0$. \\
      Satz II.16 $\implies \exists N \in \sigma(\skript{R})$ mit $N \supset (E\setminus D)$ und $\mu(E\setminus D) = \mu(N)=0 \implies D = (E\setminus N) \cup (D \cap N) \implies \skript{M}(\mu) = \overline{\sigma(\mathbb{R})}_{\mu|_{\sigma(\mathbb{R})}} \implies$ Vervollständigung von $\mu|_{\sigma(\skript{R})}$ ist $\mu|_{\skript{M}(\mu)}$. \\
      Eindeutigkeit folgt jetzt daraus und aus Satz II.15.
    \end{proof}
